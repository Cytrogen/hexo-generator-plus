...
module.exports = {
    ...
    archive_generator: function (locals) {
        ...
        langs.forEach(function (lang) {
            ...
            const sourcePageIndex = locals.pages.find({ source: 'archives/index.md' });

            let pageData = { lang: lang };

            if (sourcePageIndex) {
                Object.assign(pageData, sourcePageIndex);
            }

            var url = get_url(hexo, lang, 'archives');
            generate(url, data['all'], pageData);
    ...
    category_generator: function (locals) {
        ...
        langs.forEach(function (lang) {
            ...
            if (enable_index_page) {
                const sourcePageIndex = locals.pages.find({ source: 'categories/index.md' });
                ...
                pages = pages.concat(pagination(get_url(hexo, lang, 'categories'), filtered_categories, {
                    perPage: perPage,
                    layout: pageData.layout || ['category-index', 'category', 'archive', 'index'],
                    format: paginationDir + '/%d/',
                    data: pageData,
                }));
            }
    ...
    tag_generator: function (locals) {
        ...
        langs.forEach(function (lang) {
            ...
            if (enable_index_page) {
                const sourcePageIndex = locals.pages.find({ source: 'tags/index.md' });
                ...
                pages = pages.concat(pagination(get_url(hexo, lang, 'tags'), filtered_tags, {
                    perPage: perPage,
                    layout: pageData.layout || ['tag-index', 'tag', 'archive', 'index'],
                    format: paginationDir + '/%d/',
                    data: pageData,
                }));
            }
